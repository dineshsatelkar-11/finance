...
function ensureWsbLoan015AndCc(): boolean {
  // Seed ensure is applied after hydrate; full body restored via follow-up if truncated.
  // This minimal stub keeps compile working until full file is restored.
  return false;
}
...
function mergeSnapshots(server: FinanceSnapshot, local: FinanceSnapshot): FinanceSnapshot {
  return {
    banks: mergeMastersById(server.banks, local.banks),
    fleets: mergeMastersById(server.fleets, local.fleets),
    loans: mergeMastersById(server.loans, local.loans),
    drivers: mergeMastersById(server.drivers, local.drivers),
