...
function ensureWsbLoan015AndCc(): boolean {
  ...
  const xfers: BankTransfer[] = [
    ...
    { id: "xfer_bajaj_to_warana_5000_20260901", fromBankId: bajajId, toBankId: currentId, amount: 5000, date: "2026-09-01", note: "Bajaj → Warana · Dinesh · 01-Sep statement", createdAt: "2026-09-01T12:00:00.000Z" },
    { id: "xfer_bajaj_to_warana_5000_20260904", fromBankId: bajajId, toBankId: currentId, amount: 5000, date: "2026-09-04", note: "Bajaj → Warana · Dinesh · 04-Sep statement", createdAt: "2026-09-04T12:00:00.000Z" },
    { id: "xfer_bajaj_to_warana_20000_20260909", fromBankId: bajajId, toBankId: currentId, amount: 20000, date: "2026-09-09", note: "Bajaj → Warana · Dinesh · 09-Sep statement", createdAt: "2026-09-09T12:00:00.000Z" },
    { id: "xfer_bajaj_to_warana_2000_20260910", fromBankId: bajajId, toBankId: currentId, amount: 2000, date: "2026-09-10", note: "Bajaj → Warana · Dinesh · 10-Sep statement", createdAt: "2026-09-10T12:00:00.000Z" },
  ];
  ...
  for (const x of existing) {
    ...
    if (seed && !deletedIds.has(x.id)) {
      if (x.fromBankId !== seed.fromBankId || x.toBankId !== seed.toBankId) {
        next.push({ ...seed });
        xferChanged = true;
      } else {
        next.push(x);
  ...
  if (xferChanged) {
    useFinance.setState({ bankTransfers: next });
    changed = true;
  }
  ...
  const disbId = "lp_stmt_wsb015_disb_20260909";
  if (!deletedIds.has(disbId) && !useFinance.getState().loanPayments.some((p) => p.id === disbId)) {
    useFinance.setState((state) => ({
      loanPayments: [
        {
          id: disbId,
          loanId: WSB_LOAN_015.id,
          ...
          note: "Loan disbursement → Current · statement · then Laxmi Motors RTGS",
  ...
  const WSB_MINI_LOANS: Loan[] = [
    { id: "loan_wsb_000010", name: "WSB Mini — A/c …000010", bank: "Warana Sahakari Bank (HDFC0CSWSBL)", accountNo: "3970254350000010", ifsc: "HDFC0CSWSBL", principal: 379000, emiAmount: 7960, emiDay: 19, totalEmis: 60, startDate: "2025-11-19", endDate: "2030-11-19", interestRate: 9.5, outstanding: 332658, pendingEmis: 59, status: "active", fleetId: null, note: "SATELKARS LOGISTIC · Customer ID 554827" },
    { id: "loan_wsb_000012", name: "WSB Mini — A/c …000012", bank: "Warana Sahakari Bank (HDFC0CSWSBL)", accountNo: "3970254350000012", ifsc: "HDFC0CSWSBL", principal: 379000, emiAmount: 7960, emiDay: 13, totalEmis: 60, startDate: "2026-01-13", endDate: "2031-01-13", interestRate: 9.5, outstanding: 342955, pendingEmis: 59, status: "active", fleetId: null, note: "SATELKARS LOGISTIC · Customer ID 554827" },
    { id: "loan_wsb_000014", name: "WSB Mini — A/c …000014", bank: "Warana Sahakari Bank (HDFC0CSWSBL)", accountNo: "3970254350000014", ifsc: "HDFC0CSWSBL", principal: 388000, emiAmount: 8149, emiDay: 15, totalEmis: 60, startDate: "2026-05-15", endDate: "2031-05-15", interestRate: 9.5, outstanding: 372616, pendingEmis: 59, status: "active", fleetId: null, note: "SATELKARS LOGISTIC · Customer ID 554827" },
  ];
  for (const loan of WSB_MINI_LOANS) {
    if (!useFinance.getState().loans.some((l) => l.id === loan.id)) {
      useFinance.getState().upsertLoan(loan);
      changed = true;
    }
  }

  const emiSeeds = [
    { id: "lp_stmt_emi_000010_20260917", loanId: "loan_wsb_000010", amount: 7960, note: "SI EMI · …000010 · statement" },
    { id: "lp_stmt_emi_000012_20260917", loanId: "loan_wsb_000012", amount: 7960, note: "SI EMI · …000012 · statement" },
    { id: "lp_stmt_emi_000014_20260917", loanId: "loan_wsb_000014", amount: 8149, note: "SI EMI · …000014 · statement" },
  ];
  for (const emi of emiSeeds) {
    if (deletedIds.has(emi.id)) continue;
    if (useFinance.getState().loanPayments.some((p) => p.id === emi.id)) continue;
    if (!useFinance.getState().loans.some((l) => l.id === emi.loanId)) continue;
    useFinance.setState((state) => ({
      loanPayments: [
        {
          id: emi.id,
      ...
      loans: state.loans.map((l) =>
        l.id === emi.loanId
          ? {
              ...l,
              outstanding: Math.max(0, (l.outstanding ?? 0) - emi.amount),
              pendingEmis: Math.max(0, (l.pendingEmis ?? 0) - 1),
            }
          : l,
      ),
    }));
  ...
  const needTag = useFinance.getState().expenses.filter((e) => wegoExpIds.has(e.id) && e.fleetId !== wegoFleetId);
  if (needTag.length) {
    useFinance.setState((state) => ({
      expenses: state.expenses.map((e) => (wegoExpIds.has(e.id) ? { ...e, fleetId: wegoFleetId } : e)),
    }));
    changed = true;
  }
